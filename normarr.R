normarr=function (m, method = "ZpepQuad", robust = TRUE, centered = TRUE) 
{
  Zpep <- getZpep(method, m)
  ynew <- getWeightedEstimator(ymat = m, x = Zpep, 
                               robust, centered)
  colnames(ynew) <- colnames(m)
  if (robust) {
...
getZpep = function(method, m)
{
    X <- .makeZpepMatrix(rownames(m))
